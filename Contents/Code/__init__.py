...
class DFIAgent(Agent.Movies):
	...
	def search(self, results, media, lang):
		url = DFI_RESULT_URL % ('list?titlecontains=' + RE_REPLACE.sub(lambda m: rdict[m.group(0)], media.name.lower()))
		...
		for DFI_Results in DFI_Search:
			...
			if DFI_Results['Name'].lower() == media.name.lower():
				score = 100
			else:
				score = 100 - String.LevenshteinDistance(media.name.lower(), DFI_Results['Name'].lower())
			...
			if 'Censorship' in DFI_metadata:
				try:
					if RE_AGE.search(DFI_metadata['Censorship']):
						metadata.content_rating_age = RE_AGE.match(DFI_metadata['Censorship'])
