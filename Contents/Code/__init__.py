...
import re
DFI_SEARCH_URL = "http://nationalfilmografien.service.dfi.dk/Movie.svc/json/list?titlecontains=%s&startyear=%s"
...
class DFIAgent(Agent.Movies):
    ...
    def search(self, results, media, lang):
        DFI_Search = JSON.ObjectFromURL(DFI_SEARCH_URL % (media.name, media.year))
        for DFI_Results in DFI_Search:
            results.Append(MetadataSearchResult(id = DFI_Results['ID'], score = 100, name = DFI_Results['Name'], lang = lang))    
            Log.Info('result: ' + DFI_Results['Name'])
    ...
    def update(self, metadata, media, lang): 
        ...
        if 'Images' in DFI_metadata:
            ...
            for image in DFI_images:
                if 'ScrMini' in image and image['ImageType'] == 'poster':
                    poster = HTTP.Request(image['ScrMini'])
                    metadata.posters[image['ScrMini']] = proxy(poster, sort_order = 1)
                if 'ScrMini' in image and image['ImageType'] == 'photo':
                    art = HTTP.Request(image['ScrMini'])
                    metadata.art[image['ScrMini']] = proxy(art, sort_order = 1)
